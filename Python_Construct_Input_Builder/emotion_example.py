...
for agent_index in range(agent_count):
    for emot_index, attractor in enumerate(attractors):
        construct_xml.add_network_link(emot_net, agent_index, emot_index, value = np.random.uniform(-.1, .1) + attractor)

emot_bc_bias = construct_xml.add_network("emotion broadcast bias network", "float", "agent", "emotion")
for agent_index in range(agent_count):
    for emot_index, value in enumerate(emot_broadcast_probabilities):
...
for agent_index in range(agent_count):
    for emot_index, value in enumerate(attractors):
...
for agent_index in range(agent_count):
    for emot_index, value in enumerate(decay_rates):
